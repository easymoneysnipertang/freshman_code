...
#pragma omp for 
	for (int i = 0; i < count; i++) {// count个查询
		...
		sorted(list, *(invertedLists), num);// 按长度排序
		InvertedIndex res = ADP(list, *invertedLists, num);
		cout << i << endl;
	}
	...
	//	}
	//	sorted(list, *(invertedLists), num);// 按长度排序
	//	InvertedIndex res = ADP_dynamic(list, *invertedLists, num);
	//	cout << i << endl;
	//}
	...
	//for (int j = 0; j < NUM_THREADS; j++) {// 创建线程，传入参数
	//		param4[j].t_id = j;
	//		pthread_create(&thread[j], NULL, threadFunc4, &param4[j]);
	//}
	// 
	//for (int i = 0; i < count; i++) {// count个查询
	//	int num = 0;// query查询项数
	...
	//	}
	//	sorted(list, *(invertedLists), num);// 按长度排序
	//	InvertedIndex res = ADP_static(list, *invertedLists, num);
	//	cout << i << endl;
	// 
	//}
	// 
